...
def parse_args():
    ...
    parser.add_argument('--checkpoint_name', default='msrvtt10kmiech_model_best.pth.tar', type=str, help='name of checkpoint (default: model_best.pth.tar)')
...
def main():
    ...
    if options.space == 'hybrid':
        ...
        video_embs_all, video_ids_all = evaluation.encode_text_or_vid(model.embed_vis, vid_data_loader)
        cap_embs_all, caption_ids_all = evaluation.encode_text_or_vid(model.embed_txt, text_data_loader)
    ...
    for i in range(len(cap_embs_all)):
        ...
        if options.space in ['latent', 'hybrid']:
            t2v_all_errors_1 += evaluation.cal_error(video_embs, cap_embs, options.measure) / 4
    ...
    if options.space in ['latent', 'hybrid']:
        ...
        cal_perf(t2v_all_errors_1, v2t_all_errors=None, v2t_gt=v2t_gt, t2v_gt=t2v_gt)

    if options.space in ['concept', 'hybrid']:
        logging.info("=======Concept Space=======")
        cal_perf(t2v_all_errors_2, v2t_all_errors=None, v2t_gt=v2t_gt, t2v_gt=t2v_gt)
    
    if options.space in ['hybrid']:
        logging.info("=======Hybrid Space=======")
        w = 0.6
        ...
        cal_perf(t2v_errors, v2t_errors, v2t_gt, t2v_gt, Flags=False)
